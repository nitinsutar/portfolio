...
const projects = [
  {
    title: "VisaQuickCheck",
    description:
      "SEO-first tool that makes visa rules dead-simple per nationality. Transparent sources, clean UX.",
    tags: ["Next.js", "SEO", "Data Aggregation"],
    link: "#",
  },
  {
    title: "Vibeharbour",
    description:
      "Urban streetwear brand + site. Bold visuals, crisp copy, zero fluff.",
    tags: ["Brand", "eCom", "Design"],
    link: "#",
  },
  {
    title: "Raj Jewellers",
    description:
      "Luxury jewellery site—craftsmanship meets clean modern web.",
    tags: ["Branding", "Web Design", "Luxury"],
    link: "https://www.rajjewellersmumbai.com/",
  },
  {
    title: "OutlookMS",
    description:
      "Corporate presence done right. Straightforward, fast, professional.",
    tags: ["Corporate", "Web", "B2B"],
    link: "https://www.outlookms.com/",
  },
  {
    title: "IES MCRC",
    description:
      "Institutional site for programs, faculty, admissions—clear and accessible.",
    tags: ["Education", "UI/UX", "CMS"],
    link: "https://www.mcrc.ies.edu/",
  },
  {
    title: "Adtalk",
    description:
      "Creative ad agency site with punchy case visuals and motion.",
    tags: ["Agency", "Marketing", "Creative"],
    link: "https://theadtalk.com/",
  },
];
...
function validateProjects() {
  ...
  console.assert(projects.length >= 6, "expected at least 6 projects");
  projects.forEach((p, i) => {
    console.assert(!!p.title, `project[${i}] missing title`);
    console.assert(!!p.description, `project[${i}] missing description`);
    console.assert(Array.isArray(p.tags), `project[${i}] tags should be array`);
    console.assert(typeof p.link === "string", `project[${i}] link should be string`);
  });
}
...
export default function App() {
  ...
  return (
    <div ref={pageRef} className="min-h-screen bg-neutral-950 text-neutral-100 selection:bg-white/20">
      <motion.div style={{ width: progressWidth }} className="fixed left-0 top-0 z-50 h-1 bg-white/80" />
      <div className="fixed inset-0 -z-10">
        <div className="absolute inset-0 bg-[radial-gradient(60%_60%_at_50%_0%,rgba(120,120,255,0.12),transparent)]" />
      </div>
      ...
      <section className="relative mx-auto max-w-6xl px-4 pt-16 md:pt-24">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} style={{ y: yParallax }}>
          ...
          <h1 className="text-4xl font-semibold leading-tight tracking-tight md:text-6xl">
            <span className="text-white">Nitin Sutar</span>
            <span className="block text-white/70">Designer • Builder • Problem Solver</span>
          </h1>
          <p className="mt-6 max-w-2xl text-base text-neutral-300 md:text-lg">
            I design & ship products that feel slick and obvious. Zero hassle, max impact. If it’s not fast and clean, it’s not done.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a href="#projects" className="group inline-flex items-center rounded-full bg-white px-5 py-2 text-sm font-medium text-black transition hover:opacity-90">
              See Projects<span className="ml-2 inline-block transition group-hover:translate-x-0.5">→</span>
            </a>
        ...
        <div className="grid items-center gap-8 md:grid-cols-5">
          <div className="md:col-span-3">
            ...
            <ul className="mt-6 grid gap-2 text-sm text-white/80 md:grid-cols-2">
              <li>⚡ Rapid prototyping (days, not months)</li>
              <li>🎯 Conversion-focused UX that sells</li>
              <li>🧩 Design systems that age well</li>
              <li>🚀 SEO + Web perf baked in</li>
            </ul>
          </div>
      ...
      <section id="projects" className="mx-auto max-w-6xl px-4 py-14 md:py-20">
        <div className="mb-8 flex items-end justify-between">
          <h2 className="text-2xl font-semibold md:text-3xl">Projects</h2>
          <a href="#contact" className="text-sm text-white/70 hover:text-white">Available for collabs →</a>
        </div>
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p, idx) => (
            <motion.a key={p.title} href={p.link} target="_blank" rel="noreferrer"
              ...
              className="group relative overflow-hidden rounded-2xl border border-white/10 bg-neutral-900/50 p-5 shadow-[0_0_0_1px_rgba(255,255,255,0.03)_inset] hover:border-white/20">
              <div className="relative h-44 overflow-hidden rounded-xl bg-gradient-to-br from-neutral-800 to-neutral-900">
                {p.link !== "#" ? (
                  <iframe title={`${p.title} preview`} src={p.link} loading="lazy"
                    sandbox="allow-scripts allow-forms allow-same-origin allow-popups"
                    className="h-full w-full origin-top-left pointer-events-none opacity-90" />
                ) : (
                  <div className="flex h-full items-center justify-center text-white/30">Preview coming soon</div>
                )}
              ...
              <div className="mt-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {p.tags.map((t) => (
                    <span key={t} className="rounded-full border border-white/10 px-2 py-0.5 text-xs text-white/70">{t}</span>
                  ))}
                </div>
              </div>
            </motion.a>
